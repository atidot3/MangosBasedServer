...
SqlPreparedStatement* SqlConnection::GetStmt(uint32 nIndex)
{
	...
	if (m_holder[nIndex] == nullptr)
	{
		...
		ORIGIN_ASSERT(fmt.length());
		// allocate SQlPreparedStatement object
		pStmt = CreateStatement(fmt);
		// prepare statement
		if (!pStmt->prepare())
		{
			ORIGIN_ASSERT(false && "Unable to prepare SQL statement");
			return nullptr;
		}
...
void Database::InitDelayThread()
{
	...
	m_delayThread = new Origin::Thread(m_threadBody);
}
...
bool Database::PExecuteLog(const char* format, ...)
{
	...
	if (res == -1)
	{
		sLog.outErrorDb("SQL Query truncated (and not execute) for format: %s", format);
		return false;
	}
	...
	if (m_logSQL)
	{
		...
		else
		{
			...
			printf("SQL-Logging is disabled - Log file for the SQL commands could not be openend: %s", fName);
		}
	}
...
bool Database::BeginTransaction()
{
	...
	ORIGIN_ASSERT(!m_currentTransaction.get());   // if we will get a nested transaction request - we MUST fix code!!!
